...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        // Find user from User model by ID from Auth class
        ...
        if (isset($item['separator'])) {
            ...
            if (isset($item['permission_section']) && $user->can($item['permission_section'])) {
            ...
            if (isset($item['permission']) && $user->can($item['permission']) OR $item['permission'] == false) {
                ...
                if (isset($item['parent'])) {
                    ...
                    if(isset($item['page_up']))
                    {
                        if(request()->path() == $item['page_up'])
                            $url;

                        $url = url('/'. Auth::user()->roles->pluck('name')->first());
                    }
